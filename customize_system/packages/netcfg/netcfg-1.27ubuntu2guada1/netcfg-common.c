...
int netcfg_get_domain(struct debconfclient *client,  char **domain)
{
    ...
    debconf_input (client, "medium", "netcfg/get_domain");
    ret = debconf_go(client);
...
void netcfg_write_common(struct in_addr ipaddress, char *hostname, char *domain)
{
    ...
    if ((fp = file_open(IFTAB_FILE, "w"))) {
        ...
        ifr = malloc(sizeof(struct ifreq) * num_interfaces);
        for(i = 0 ; i < num_interfaces; ++i) {
            memset(&ifr[i], 0, sizeof(struct ifreq));

            strncpy(ifr[i].ifr_name, ifs[i], IFNAMSIZ);
            ifr[i].ifr_name[IFNAMSIZ-1] = '\0';
        
            if (ioctl(sock, SIOCGIFHWADDR, &ifr[i]) != 0)
                ifr[i].ifr_hwaddr.sa_family = -1;
        }

        close(sock);


        fprintf(fp, "# This file assigns persistent names to network interfaces.\n");
        fprintf(fp, "# See iftab(5) for syntax.\n\n");
        ...
        for(i = 0; i < num_interfaces; i++) {
            ...
            /* Check no other interface has this address */
            for (ii = 0; ii < num_interfaces; ii++) {
                if (i == ii)
                    continue;
                if (memcmp(ifr[i].ifr_hwaddr.sa_data,
                           ifr[ii].ifr_hwaddr.sa_data, 6))
                    continue;

                if (ifr[ii].ifr_hwaddr.sa_family == ARPHRD_ETHER)
                    dup = 1;
            }
            ...
            fprintf(fp, "%s mac %02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx arp %d\n",
                    ifr[i].ifr_name,
                    ifr[i].ifr_hwaddr.sa_data[0],
                    ifr[i].ifr_hwaddr.sa_data[1],
                    ifr[i].ifr_hwaddr.sa_data[2],
                    ifr[i].ifr_hwaddr.sa_data[3],
                    ifr[i].ifr_hwaddr.sa_data[4],
                    ifr[i].ifr_hwaddr.sa_data[5],
		    ifr[i].ifr_hwaddr.sa_family);
        }
